...
local dexes = {}
local _M = {}
local luacontext = jf--activity or service
...
-- jc.load = function(packageName){
--     append(loaded,jc.getClass().getClassLoader().loadClass())
-- }
...
local function import_path_class(packageName)
    -- body
    local res, class = pcall(jc.getClass().getClassLoader().loadClass, packagename)
        if res then
            loaded[packagename] = class
            return class
        end
...
local function local_import(_env, packages, package)
    ...
    if i then -- a wildcard; put into the package list, including the final '.'
        ...
        local classname = package:match('([^%.$]+)$')
        local class = import_require(package) or import_class(package) or import_dex_class(package) or import_path_class(package)
        if class then
            if class ~= true then
...
function _M.compile(dexname)
    append(dexes, jc.load(dexname))
end
